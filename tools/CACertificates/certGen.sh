...
function warn_certs_not_for_production()
{
    tput smso 
    tput setaf 3 
    echo "Certs generated by this script are not for production (e.g. they have hard-coded passwords of '123'."
...
{  
    ...
    local intermediate_ca_prefix="azure-iot-test-only.intermediate"
...
###############################################################################
# Generate a Certificate for a leaf, signed with either the root or intermediate cert.
###############################################################################
...
{    
    ...
    echo "Create the Device Certificate Request"
    echo "----------------------------------------"
    ...
    openssl ca -batch -config ${openssl_config_file} \
            ${password_cmd} \
            -extensions server_cert -days ${days_till_expire} -notext -md sha256 \
    ...
    echo "Verify signature of the leaf certificate with the signer"
    echo "-----------------------------------"
    openssl verify \
    ...
    echo "Device Certificate Generated At:"
    echo "----------------------------------------"
    ...
    echo "Create the Device PFX Certificate"
    echo "----------------------------------------"
    ...
    echo "Device PFX Certificate Generated At:"
    echo "--------------------------------------------"
...
function generate_verification_certificate()
{
    ...
    generate_leaf_certificate "${1}" "verification-code" ${root_ca_dir} ${root_ca_password} ${openssl_root_config_file}
}
