...
static const SEC_TPM_INTERFACE sec_tpm_interface =
{
    dps_hsm_tpm_create,
    dps_hsm_tpm_destroy,
    dps_hsm_tpm_import_key,
    dps_hsm_tpm_get_endorsement_key,
    dps_hsm_tpm_get_storage_key,
    dps_hsm_tpm_sign_data
};
...
static int create_tpm_session(DPS_SECURE_DEVICE_INFO* sec_info, TSS_SESSION* tpm_session)
{
    int result;
...
static int insert_key_in_tpm(DPS_SECURE_DEVICE_INFO* sec_info, const unsigned char* key, size_t key_len)
{
    int result;
...
static BUFFER_HANDLE decrypt_data(DPS_SECURE_DEVICE_INFO* sec_info, const unsigned char* key, size_t key_len)
{
...
static int create_persistent_key(DPS_SECURE_DEVICE_INFO* tpm_info, TPM_HANDLE request_handle, TPMI_DH_OBJECT hierarchy, TPM2B_PUBLIC* inPub, TPM2B_PUBLIC* outPub)
{
    int result;
    TPM_RC tpm_result;
    TPM2B_NAME name;
...
static int initialize_tpm_device(DPS_SECURE_DEVICE_INFO* tpm_info)
{
    int result;
    ...
    /* Codes_SRS_SECURE_DEVICE_TPM_07_030: [ secure_dev_tpm_create shall call into the tpm_codec to initialize a TSS session. ] */
    else if (Initialize_TPM_Codec(&tpm_info->tpm_device) != TPM_RC_SUCCESS)
    {
        LogError("Failure initializeing TPM Codec");
        result = __FAILURE__;
    }
    /* Codes_SRS_SECURE_DEVICE_TPM_07_031: [ secure_dev_tpm_create shall get a handle to the Endorsement Key and Storage Root Key. ] */
...
DPS_SECURE_DEVICE_HANDLE dps_hsm_tpm_create()
{
    DPS_SECURE_DEVICE_INFO* result;
    /* Codes_SRS_SECURE_DEVICE_TPM_07_002: [ On success dps_hsm_tpm_create shall allocate a new instance of the secure device tpm interface. ] */
    result = malloc(sizeof(DPS_SECURE_DEVICE_INFO) );
    if (result == NULL)
    {
        /* Codes_SRS_SECURE_DEVICE_TPM_07_001: [ If any failure is encountered dps_hsm_tpm_create shall return NULL ] */
        LogError("Failure: malloc DPS_SECURE_DEVICE_INFO.");
    }
    else
    {
        memset(result, 0, sizeof(DPS_SECURE_DEVICE_INFO));
        /* Codes_dps_hsm_tpm_create shall call into the tpm_codec to initialize a TSS session.*/
...
void dps_hsm_tpm_destroy(DPS_SECURE_DEVICE_HANDLE handle)
{
    /* Codes_SRS_SECURE_DEVICE_TPM_07_005: [ if handle is NULL, dps_hsm_tpm_destroy shall do nothing. ] */
    if (handle != NULL)
    {
        /* Codes_SRS_SECURE_DEVICE_TPM_07_004: [ dps_hsm_tpm_destroy shall free the DPS_SECURE_DEVICE_INFO instance. ] */
        Deinit_TPM_Codec(&handle->tpm_device);
        /* Codes_SRS_SECURE_DEVICE_TPM_07_006: [ dps_hsm_tpm_destroy shall free all resources allocated in this module. ]*/
        free(handle);
    }
}

int initialize_tpm_system(void)
{
    return 0;
}

void deinitialize_tpm_system(void)
{
}

int dps_hsm_tpm_import_key(DPS_SECURE_DEVICE_HANDLE handle, const unsigned char* key, size_t key_len)
{
    int result;
    if (handle == NULL || key == NULL || key_len == 0)
    {
        /* Codes_SRS_SECURE_DEVICE_TPM_07_007: [ if handle or key are NULL, or key_len is 0 dps_hsm_tpm_import_key shall return a non-zero value ] */
        LogError("Invalid argument specified handle: %p, key: %p, key_len: %zu", handle, key, key_len);
        result = __FAILURE__;
    }
    else
    {
        /* Codes_dps_hsm_tpm_import_key shall establish a tpm session in preparation to inserting the key into the tpm. */
        ...
        {
            /* Codes_SRS_SECURE_DEVICE_TPM_07_008: [ On success dps_hsm_tpm_import_key shall return zero ] */
            result = 0;
        }
    }
    return result;
...
BUFFER_HANDLE dps_hsm_tpm_get_endorsement_key(DPS_SECURE_DEVICE_HANDLE handle)
{
    BUFFER_HANDLE result;
    if (handle == NULL)
    {
        /* Codes_SRS_SECURE_DEVICE_TPM_07_013: [ If handle is NULL dps_hsm_tpm_get_endorsement_key shall return NULL. ] */
        LogError("Invalid handle value specified");
        result = NULL;
    }
    else if (handle->ek_pub.publicArea.unique.rsa.t.size == 0)
    {
        /* Codes_SRS_SECURE_DEVICE_TPM_07_027: [ If the ek_public was not initialized dps_hsm_tpm_get_endorsement_key shall return NULL. ] */
    ...
    else
    {
        ...
        /* Codes_SRS_SECURE_DEVICE_TPM_07_014: [ dps_hsm_tpm_get_endorsement_key s hall allocate and return the Endorsement Key. ] */
        uint32_t data_length = TPM2B_PUBLIC_Marshal(&handle->ek_pub, &data_pos, NULL);
        result = BUFFER_create(data_bytes, data_length);
        if (result == NULL)
        {
            /* Codes_SRS_SECURE_DEVICE_TPM_07_015: [ If a failure is encountered, dps_hsm_tpm_get_endorsement_key shall return NULL. ] */
...
BUFFER_HANDLE dps_hsm_tpm_get_storage_key(DPS_SECURE_DEVICE_HANDLE handle)
{
    BUFFER_HANDLE result;
    if (handle == NULL)
    {
        /* Codes_SRS_SECURE_DEVICE_TPM_07_016: [ If handle is NULL, dps_hsm_tpm_get_storage_key shall return NULL. ] */
        LogError("Invalid handle value specified");
        result = NULL;
    }
    else if (handle->srk_pub.publicArea.unique.rsa.t.size == 0)
    {
        /* Codes_SRS_SECURE_DEVICE_TPM_07_017: [ If the srk_public value was not initialized, dps_hsm_tpm_get_storage_key shall return NULL. ] */
    ...
    else
    {
        unsigned char data_bytes[1024];
        unsigned char* data_pos = data_bytes;
        /* Codes_SRS_SECURE_DEVICE_TPM_07_018: [ dps_hsm_tpm_get_storage_key shall allocate and return the Storage Root Key. ] */
        uint32_t data_length = TPM2B_PUBLIC_Marshal(&handle->srk_pub, &data_pos, NULL);
        result = BUFFER_create(data_bytes, data_length);
        if (result == NULL)
        {
            /* Codes_SRS_SECURE_DEVICE_TPM_07_019: [ If any failure is encountered, dps_hsm_tpm_get_storage_key shall return NULL. ] */
...
BUFFER_HANDLE dps_hsm_tpm_sign_data(DPS_SECURE_DEVICE_HANDLE handle, const unsigned char* data, size_t data_len)
{
    BUFFER_HANDLE result;
    /* Codes_SRS_SECURE_DEVICE_TPM_07_020: [ If handle or data is NULL or data_len is 0, dps_hsm_tpm_sign_data shall return NULL. ] */
    ...
    else
    {
        ...
        /* Codes_SRS_SECURE_DEVICE_TPM_07_021: [ dps_hsm_tpm_sign_data shall call into the tpm to hash the supplied data value. ] */
        uint32_t sign_len = SignData(&handle->tpm_device, &NullPwSession, data_copy, (UINT32)data_len, data_signature, sizeof(data_signature) );
        if (sign_len == 0)
        {
            /* Codes_SRS_SECURE_DEVICE_TPM_07_023: [ If an error is encountered dps_hsm_tpm_sign_data shall return NULL. ] */
            LogError("Failure signing data from hash");
            result = NULL;
        }
        else
        {
            /* Codes_SRS_SECURE_DEVICE_TPM_07_022: [ If hashing the data was successful, dps_hsm_tpm_sign_data shall create a BUFFER_HANDLE with the supplied signed data. ] */
...
/* Codes_SRS_SECURE_DEVICE_TPM_07_026: [ dps_hsm_tpm_interface shall return the SEC_TPM_INTERFACE structure. ] */
const SEC_TPM_INTERFACE* dps_hsm_tpm_interface()
{
