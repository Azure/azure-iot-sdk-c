...
static Car* parse_from_cbor(const char* json, DEVICE_TWIN_UPDATE_STATE update_state)
{
    ...
    else
    {
        ...
        root_value = json_parse_string(cbor);
        root_object = json_value_get_object(root_value);

        CborEncoder encoder;
        decode_json(json, &encoder);
        CborValue cbor_val;
...
static void device_twin_callback(DEVICE_TWIN_UPDATE_STATE update_state, const unsigned char* payLoad, size_t size, void* userContextCallback)
{
    ...
    Car* newCar = parse_from_cbor((const char*)payLoad, update_state);
...
static void iothub_client_device_twin_and_methods_sample_run(void)
{
    ...
    else
    {
        ...
        else
        {
            ...
            char* reportedProperties = serialize_to_cbor(&car);
