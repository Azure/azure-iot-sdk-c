...
// Tests_SRS_IOTHUB_CLIENT_RETRY_CONTROL_09_023: [If `retry_control->max_retry_time_in_secs` is not 0 and (`current_time` - `retry_control->initial_retry_time`) is greater or equal to `retry_control->max_retry_time_in_secs`, `retry_action` shall be set to RETRY_ACTION_STOP_RETRYING]
...
// Tests_SRS_IOTHUB_CLIENT_RETRY_CONTROL_09_011: [If `retry_control->initial_retry_time` is INDEFINITE_TIME, it shall be set using get_time()]
...
// Tests_SRS_IOTHUB_CLIENT_RETRY_CONTROL_09_035: [`retry_control` shall have fields `retry_count` and `current_wait_time_in_secs` set to 0 (zero), `initial_retry_time` and `last_retry_time` set to INDEFINITE_TIME]
