...
{
    STRICT_EXPECTED_CALL(IoTHubClientCore_LL_GetTransportCallbacks(IGNORED_ARG));
...
TEST_FUNCTION(IoTHubTransport_StartWorkerThread_success)
{
    ...
    STRICT_EXPECTED_CALL(ThreadAPI_Create(IGNORED_ARG, IGNORED_ARG, handle));
    STRICT_EXPECTED_CALL(Lock(IGNORED_ARG));
...
TEST_FUNCTION(IoTHubTransport_StartWorkerThread_client_call_twice_success)
{
    ...
    STRICT_EXPECTED_CALL(Lock(IGNORED_ARG));
    STRICT_EXPECTED_CALL(VECTOR_size(IGNORED_ARG));
    STRICT_EXPECTED_CALL(VECTOR_find_if(IGNORED_ARG, IGNORED_ARG, TEST_IOTHUB_CLIENT_CORE_HANDLE1));
    STRICT_EXPECTED_CALL(Unlock(IGNORED_ARG));
...
TEST_FUNCTION(IoTHubTransport_StartWorkerThread_client_call_new_client_success)
{
    ...
    STRICT_EXPECTED_CALL(Lock(IGNORED_ARG));
    STRICT_EXPECTED_CALL(VECTOR_size(IGNORED_ARG));
    STRICT_EXPECTED_CALL(VECTOR_find_if(IGNORED_ARG, IGNORED_ARG, TEST_IOTHUB_CLIENT_CORE_HANDLE2));
    STRICT_EXPECTED_CALL(VECTOR_push_back(IGNORED_ARG, IGNORED_ARG, 1));
    STRICT_EXPECTED_CALL(Unlock(IGNORED_ARG));
...
TEST_FUNCTION(IoTHubTransport_worker_thread_runs_every_1_ms)
{
    ...
    for (size_t index = 0; index < g_how_many_dowork_calls+1; index++)
    {
        ...
        if (index != g_how_many_dowork_calls)
        {
            STRICT_EXPECTED_CALL(VECTOR_element(IGNORED_ARG, IGNORED_ARG));
        }
        STRICT_EXPECTED_CALL(Unlock(IGNORED_ARG));
    ...
    STRICT_EXPECTED_CALL(Lock(IGNORED_ARG));
    STRICT_EXPECTED_CALL(Unlock(IGNORED_ARG));
    STRICT_EXPECTED_CALL(ThreadAPI_Exit(IGNORED_ARG));
