...
* Exposes a platform abstraction layer to isolate OS dependencies (threading and mutual exclusion mechanisms, communications protocol e.g. HTTP). Refer to [porting guide][c-porting-guide] for more information.
...
At this point you can use the packages on Ubuntu 14.04, 15.04, 15.10, 16.04 using an x86 CPU architecture.
...
If you are working with a device with a different Linux distribution or with an x64 or ARM CPU, then you will need to compile the SDK following the instructions below.
...
For developers creating device applications on the [mbed](http://mbed.org) platform, we have a published a library and samples that will get you started in minutes witH Azure IoT Hub. This library and the samples have been tested with the following boards:
* Freescale FRDMK64-F
* Renesas GR-PEACH
* SADE.IO GSM Gateway

To use the samples and the Azure IoT device SDK library in your mbed applications, follow these steps:
...
You can find detailed instructions for each of the tested devices in our [device catalog][device-catalog]:
...
* [Cross compile the C SDK (targeting Raspbian and using Ubuntu as host)](../doc/SDK_cross_compile_example.md)

## Samples

The repository contains a set of simple samples that will help you get started.
You can find a list of these samples with instructrions on how to run them [here][samples]. 
