...
#define IOTHUB_CLIENT_FILE_UPLOAD_GET_DATA_RESULT_VALUES \
    ...
    */
    ...
    /** @brief    This struct specifies parameters of an incoming command response */  
    typedef struct IOTHUB_CLIENT_COMMAND_RESPONSE_TAG
    {
        ...
        /** @brief Number of bytes application is returning in response.  This should *not* include the terminating null. */
        size_t payloadLength;
        /** @brief Status code of the command to return to the service.  This maps to an HTTP style status code. */
        int statusCode;
    ...
    *
    * @param[in]    commandRequest        Parameters specified by IoT Hub for this command's request.
    * @param[in]    commandResponse       Response filled in by application callback to the command request.
