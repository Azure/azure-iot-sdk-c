...
typedef struct IOTHUB_DESERIALIZED_PROPERTY_TAG {
    /** @brief   Version of the structure.  Currently must be IOTHUB_DESERIALIZED_PROPERTY_VERSION. */
    int version;
...
*           alternative, so the application can use the more convenient IOTHUB_REPORTED_PROPERTY structure instead of raw serialization.
* 
*           This API does not perform any network I/O.  It only translates IOTHUB_REPORTED_PROPERTY into a byte array.  Applications
...
*           alternative, so the application can use the more convenient IOTHUB_WRITEABLE_PROPERTY_RESPONSE structure instead of raw serialization.
*
*           This API does not perform any network I/O.  It only translates IOTHUB_WRITEABLE_PROPERTY_RESPONSE into a byte array.  Applications
*           should use APIs such as IoTHubDeviceClient_LL_SendPropertiesAsync to send the serialized data.
*
* @return   IOTHUB_CLIENT_OK upon success or an error code upon failure.
*/
...
/**
* @brief   Converts the raw payload of properties received from the service into IOTHUB_DESERIALIZED_PROPERTY struct(s).
*
