		...
		if (result != AMQP_MESSENGER_SEND_RESULT_SUCCESS)
		{
			if (twin_op_ctx->type == TWIN_OPERATION_TYPE_PATCH)
			{
				...
				if (twin_op_ctx->cb.reported_properties.callback != NULL)
				{
					...
					twin_op_ctx->cb.reported_properties.callback(TWIN_REPORT_STATE_RESULT_ERROR, TWIN_REPORT_STATE_REASON_NONE, 0, NULL, (void*)twin_op_ctx->cb.reported_properties.context);
				}
			}
...
static int send_twin_operation_request(TWIN_MESSENGER_INSTANCE* twin_msgr, TWIN_OPERATION_CONTEXT* op_ctx, CONSTBUFFER_HANDLE data, char* version)
{
	int result;
	MESSAGE_HANDLE amqp_message;

	if ((amqp_message = create_amqp_message_for_twin_operation(op_ctx->type, op_ctx->correlation_id, data, version)) == NULL)
	{
	...
	else
	{
		...
		if (get_difftime(current_time, twin_patch_ctx->time_enqueued) >= DEFAULT_TWIN_OPERATION_TIMEOUT_SECS)
		{
			...
			*continue_processing = true;
			...
			{
				twin_patch_ctx->on_report_state_complete_callback(TWIN_REPORT_STATE_RESULT_ERROR, TWIN_REPORT_STATE_REASON_TIMEOUT, 0, NULL, twin_patch_ctx->on_report_state_complete_context);
			}
		...
		else
		{
			...
			*continue_processing = true;
			...
			if (twin_op_ctx->type == TWIN_OPERATION_TYPE_PATCH)
			{
				...
				if (twin_op_ctx->cb.reported_properties.callback != NULL)
				{
					twin_op_ctx->cb.reported_properties.callback(TWIN_REPORT_STATE_RESULT_ERROR, TWIN_REPORT_STATE_REASON_TIMEOUT, 0, NULL, twin_op_ctx->cb.reported_properties.context);
				}
			}
...
static bool send_pending_twin_patch(const void* item, const void* match_context, bool* continue_processing)
{
	...
	else
	{
		...
		if ((twin_op_ctx = create_twin_operation_context(twin_msgr, TWIN_OPERATION_TYPE_PATCH)) == NULL)
		{
			...
			if (twin_patch_ctx->on_report_state_complete_callback != NULL)
			{
				twin_patch_ctx->on_report_state_complete_callback(TWIN_REPORT_STATE_RESULT_ERROR, TWIN_REPORT_STATE_REASON_INTERNAL_ERROR, 0, NULL, twin_patch_ctx->on_report_state_complete_context);
			}
		}
		...
		else
		{
			...
			if (send_twin_operation_request(twin_msgr, twin_op_ctx, twin_patch_ctx->data, twin_patch_ctx->version) != RESULT_OK)
			{
				...
				if (twin_patch_ctx->on_report_state_complete_callback != NULL)
				{
					twin_patch_ctx->on_report_state_complete_callback(TWIN_REPORT_STATE_RESULT_ERROR, TWIN_REPORT_STATE_REASON_FAIL_SENDING, 0, NULL, twin_patch_ctx->on_report_state_complete_context);
				}
...
static void process_twin_subscription(TWIN_MESSENGER_INSTANCE* twin_msgr)
{
	...
	else
	{
		...
		{
			op_type = TWIN_OPERATION_TYPE_DELETE;
		...
		if (next_subscription_state != twin_msgr->subscription_state)
		{
			...
			else
			{
				...
				else if (send_twin_operation_request(twin_msgr, twin_op_ctx, NULL, NULL) != RESULT_OK)
				{
...
static bool cancel_all_pending_twin_operations(const void* item, const void* match_context, bool* continue_processing)
{
	...
	else
	{
		...
		if (twin_op_ctx->type == TWIN_OPERATION_TYPE_PATCH)
		{
			if (twin_op_ctx->cb.reported_properties.callback != NULL)
			{
				twin_op_ctx->cb.reported_properties.callback(TWIN_REPORT_STATE_RESULT_CANCELLED, TWIN_REPORT_STATE_REASON_MESSENGER_DESTROYED, 0, NULL, twin_op_ctx->cb.reported_properties.context);
			}
		}
...
static bool cancel_pending_twin_patch_operation(const void* item, const void* match_context, bool* continue_processing)
{
	...
	else
	{
		...
		if (twin_patch_ctx->on_report_state_complete_callback != NULL)
		{
			twin_patch_ctx->on_report_state_complete_callback(TWIN_REPORT_STATE_RESULT_CANCELLED, TWIN_REPORT_STATE_REASON_MESSENGER_DESTROYED, 0, NULL, twin_patch_ctx->on_report_state_complete_context);
		}
					...
					{	
						if (twin_op_ctx->type == TWIN_OPERATION_TYPE_PATCH)
						{
							...
							if (!has_status_code)
							{
								...
								if (twin_op_ctx->cb.reported_properties.callback != NULL)
								{
									twin_op_ctx->cb.reported_properties.callback(TWIN_REPORT_STATE_RESULT_ERROR, TWIN_REPORT_STATE_REASON_INVALID_RESPONSE, 0, new_version, twin_op_ctx->cb.reported_properties.context);
								}
							}
							else
							{
								...
								if (twin_op_ctx->cb.reported_properties.callback != NULL)
								{
                                    twin_op_ctx->cb.reported_properties.callback(TWIN_REPORT_STATE_RESULT_SUCCESS, TWIN_REPORT_STATE_REASON_NONE, status_code, new_version, twin_op_ctx->cb.reported_properties.context);
								}
							}
						}
						else if (twin_op_ctx->type == TWIN_OPERATION_TYPE_GET)
...
int twin_messenger_report_state_async(TWIN_MESSENGER_HANDLE twin_msgr_handle, CONSTBUFFER_HANDLE data, const char* version, TWIN_MESSENGER_REPORT_STATE_COMPLETE_CALLBACK on_report_state_complete_callback, const void* context)
{
	...
	else
	{
        ...
        {
            memset(twin_patch_ctx, 0, sizeof(TWIN_PATCH_OPERATION_CONTEXT));

            // Codes_IOTHUBTRANSPORT_AMQP_TWIN_MESSENGER_09_025: [`twin_op_ctx` shall have a copy of `data`]  
            if ((twin_patch_ctx->data = CONSTBUFFER_Clone(data)) == NULL)
            {
