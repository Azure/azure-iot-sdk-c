...
static void attach_device_client_type_to_link(LINK_HANDLE link, STRING_HANDLE product_info)
{
    ...
    else
    {
        ...
        else
        {
            ...
            {
                int result = amqpvalue_set_map_value(attach_properties, device_client_type_key_name, device_client_type_value);
                if (result != 0)
                {
                ...
                {
                    result = link_set_attach_properties(link, attach_properties);
                    if (result != 0)
                    {
...
static AMQP_VALUE on_message_received_internal_callback(const void* context, MESSAGE_HANDLE message)
{
    ...
    int api_call_result = message_create_IoTHubMessage_from_uamqp_message(message, &iothub_message);
...
static void invoke_callback(const void* item, const void* action_context, bool* continue_processing)
{
    ...
    {
        // Typecast to smaller type on 64 bit systems, since we control ultimate caller.
        TELEMETRY_MESSENGER_EVENT_SEND_COMPLETE_RESULT messenger_send_result =
            (TELEMETRY_MESSENGER_EVENT_SEND_COMPLETE_RESULT)(((intptr_t)action_context) & 0xff);
