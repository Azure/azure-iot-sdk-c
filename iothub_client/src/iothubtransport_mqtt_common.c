        ...
        else
        {
            ...
            else
            {
                ...
                else
                {
                    ...
                    else if (extractMqttProperties(IoTHubMessage, topic_resp) != 0)
                    {
                    ...
                    else
                    {
                        ...
                        else
                        {
                            ...
                            {
                                // Codes_SRS_IOTHUB_MQTT_TRANSPORT_31_065: [ If type is IOTHUB_TYPE_TELEMETRY and sent to an input queue, then on success `mqtt_notification_callback` shall call `IoTHubClient_LL_MessageCallbackFromInput`. ]
                                if (!IoTHubClient_LL_MessageCallbackFromInput(transportData->llClientHandle, messageData))
                                {
...
static int SendMqttConnectMsg(PMQTTTRANSPORT_HANDLE_DATA transport_data)
{
    ...
    {
        sasToken = IoTHubClient_Auth_Get_SasToken(transport_data->authorization_module, STRING_c_str(transport_data->devicesAndModulesPath), transport_data->option_sas_token_lifetime_secs);
        if (sasToken == NULL)
        {
