...
#define SYS_PROP_COMPONENT_NAME "sub"
...
static int addSystemPropertiesTouMqttMessage(IOTHUB_MESSAGE_HANDLE iothub_message_handle, STRING_HANDLE topic_string, size_t* index_ptr, bool urlencode)
{
    ...
    if (result == 0)
    {
        ...
        {
            if (addSystemPropertyToTopicString(topic_string, index, SYS_PROP_COMPONENT_NAME, component_name, urlencode) != 0)
            {
                LogError("Failed setting component name name");
                result = MU_FAILURE;   
            }
            else
            {
