...
static void retry_control_destroy_option(const char* name, const void* value)
{
	if ((name == NULL) || (value == NULL))
	{
		...
		strcmp(RETRY_CONTROL_OPTION_MAX_JITTER_PERCENT, name) == 0)
	{
		// these are handles to unsigned int variables, so nothing needs to be destroyed.
	}
}
...
static int evaluate_retry_action(RETRY_CONTROL_INSTANCE* retry_control, RETRY_ACTION *retry_action)
{
	...
	// Codes_SRS_IOTHUB_CLIENT_RETRY_CONTROL_09_020: [If `retry_control->last_retry_time` is INDEFINITE_TIME and policy is not IOTHUB_CLIENT_RETRY_IMMEDIATE, the evaluation function shall return non-zero]
		     ...
		     retry_control->policy_name != IOTHUB_CLIENT_RETRY_IMMEDIATE)
	{
	...
	else
	{
		...
		// Codes_SRS_IOTHUB_CLIENT_RETRY_CONTROL_09_023: [If `retry_control->max_retry_time_in_secs` is not 0 and (`current_time` - `retry_control->initial_retry_time`) is greater or equal to `retry_control->max_retry_time_in_secs`, `retry_action` shall be set to RETRY_ACTION_STOP_RETRYING]
		else if (retry_control->max_retry_time_in_secs > 0 &&
			get_difftime(current_time, retry_control->initial_retry_time) >= retry_control->max_retry_time_in_secs)
		{
			*retry_action = RETRY_ACTION_STOP_RETRYING;
		...
		// Codes_SRS_IOTHUB_CLIENT_RETRY_CONTROL_09_028: [If `retry_control->policy_name` is IOTHUB_CLIENT_RETRY_IMMEDIATE, retry_action shall be set to RETRY_ACTION_RETRY_NOW]
		else if (retry_control->policy_name == IOTHUB_CLIENT_RETRY_IMMEDIATE)
		{
			*retry_action = RETRY_ACTION_RETRY_NOW;
...
static unsigned int calculate_next_wait_time(RETRY_CONTROL_INSTANCE* retry_control)
{
	...
	// Codes_SRS_IOTHUB_CLIENT_RETRY_CONTROL_09_029: [If `retry_control->policy_name` is IOTHUB_CLIENT_RETRY_INTERVAL, calculate_next_wait_time shall return `retry_control->initial_wait_time_in_secs`]
	if (retry_control->policy_name == IOTHUB_CLIENT_RETRY_INTERVAL)
	{
		result = retry_control->initial_wait_time_in_secs;
	}
	// Codes_SRS_IOTHUB_CLIENT_RETRY_CONTROL_09_030: [If `retry_control->policy_name` is IOTHUB_CLIENT_RETRY_LINEAR_BACKOFF, calculate_next_wait_time shall return (`retry_control->initial_wait_time_in_secs` * (`retry_control->retry_count`))]
	else if (retry_control->policy_name == IOTHUB_CLIENT_RETRY_LINEAR_BACKOFF)
	{
		result = retry_control->initial_wait_time_in_secs * (retry_control->retry_count);
	}
	// Codes_SRS_IOTHUB_CLIENT_RETRY_CONTROL_09_031: [If `retry_control->policy_name` is IOTHUB_CLIENT_RETRY_EXPONENTIAL_BACKOFF, calculate_next_wait_time shall return (pow(2, `retry_control->retry_count` - 1) * `retry_control->initial_wait_time_in_secs`)]
	else if (retry_control->policy_name == IOTHUB_CLIENT_RETRY_EXPONENTIAL_BACKOFF)
	{
		result = (unsigned int)(pow(2, retry_control->retry_count - 1) * retry_control->initial_wait_time_in_secs);
	}
	// Codes_SRS_IOTHUB_CLIENT_RETRY_CONTROL_09_032: [If `retry_control->policy_name` is IOTHUB_CLIENT_RETRY_EXPONENTIAL_BACKOFF_WITH_JITTER, calculate_next_wait_time shall return ((pow(2, `retry_control->retry_count` - 1) * `retry_control->initial_wait_time_in_secs`) * (1 + (`retry_control->max_jitter_percent` / 100) * (rand() / RAND_MAX)))]
	else if (retry_control->policy_name == IOTHUB_CLIENT_RETRY_EXPONENTIAL_BACKOFF_WITH_JITTER)
	{
	...
	// Codes_SRS_IOTHUB_CLIENT_RETRY_CONTROL_09_033: [If `retry_control->policy_name` is IOTHUB_CLIENT_RETRY_RANDOM, calculate_next_wait_time shall return (`retry_control->initial_wait_time_in_secs` * (rand() / RAND_MAX))]
	else if (retry_control->policy_name == IOTHUB_CLIENT_RETRY_RANDOM)
	{
	...
	{
		LogError("Failed to calculate the next wait time (policy %d is not expected)", retry_control->policy_name);
...
void retry_control_reset(RETRY_CONTROL_HANDLE retry_control_handle)
{
	...
	else
	{
		...
		// Codes_SRS_IOTHUB_CLIENT_RETRY_CONTROL_09_035: [`retry_control` shall have fields `retry_count` and `current_wait_time_in_secs` set to 0 (zero), `initial_retry_time` and `last_retry_time` set to INDEFINITE_TIME]
		retry_control->retry_count = 0;
		retry_control->current_wait_time_in_secs = 0;
		retry_control->initial_retry_time = INDEFINITE_TIME;
...
RETRY_CONTROL_HANDLE retry_control_create(IOTHUB_CLIENT_RETRY_POLICY policy_name, unsigned int max_retry_time_in_secs)
{
	...
	else
	{
		...
		// Codes_SRS_IOTHUB_CLIENT_RETRY_CONTROL_09_005: [If `policy_name` is IOTHUB_CLIENT_RETRY_EXPONENTIAL_BACKOFF or IOTHUB_CLIENT_RETRY_EXPONENTIAL_BACKOFF_WITH_JITTER, `retry_control->initial_wait_time_in_secs` shall be set to 1]
		if (retry_control->policy_name == IOTHUB_CLIENT_RETRY_EXPONENTIAL_BACKOFF ||
			retry_control->policy_name == IOTHUB_CLIENT_RETRY_EXPONENTIAL_BACKOFF_WITH_JITTER)
		{
...
int retry_control_should_retry(RETRY_CONTROL_HANDLE retry_control_handle, RETRY_ACTION *retry_action)
{
	...
	else
	{
		...
		// Codes_SRS_IOTHUB_CLIENT_RETRY_CONTROL_09_027: [If `retry_control->policy_name` is IOTHUB_CLIENT_RETRY_NONE, retry_action shall be set to RETRY_ACTION_STOP_RETRYING and return immediatelly with result 0]
		if (retry_control->policy_name == IOTHUB_CLIENT_RETRY_NONE)
		{
			*retry_action = RETRY_ACTION_STOP_RETRYING;
			result = RESULT_OK;
		}
		// Codes_SRS_IOTHUB_CLIENT_RETRY_CONTROL_09_011: [If `retry_control->initial_retry_time` is INDEFINITE_TIME, it shall be set using get_time()]
		else if (retry_control->initial_retry_time == INDEFINITE_TIME && (retry_control->initial_retry_time = get_time(NULL)) == INDEFINITE_TIME)
		{
		...
		else
		{
			if (*retry_action == RETRY_ACTION_RETRY_NOW)
			{
				...
				if (retry_control->policy_name != IOTHUB_CLIENT_RETRY_IMMEDIATE)
				{
