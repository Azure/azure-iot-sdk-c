...
// this function should split a file in as many pieces as necessary
...
IOTHUB_CLIENT_RESULT IoTHubClient_LL_UploadToBlob_Impl(IOTHUB_CLIENT_LL_UPLOADTOBLOB_HANDLE handle, const char* destinationFileName, const unsigned char* source, size_t size)
{
    ...
    result = IoTHubClient_LL_UploadToBlob_WithCallback_Impl(handle, destinationFileName, FileUpload_GetData_Callback, context);

    return result;
}

IOTHUB_CLIENT_RESULT IoTHubClient_LL_UploadToBlob_WithCallback_Impl(IOTHUB_CLIENT_LL_UPLOADTOBLOB_HANDLE handle, const char* destinationFileName, IOTHUB_CLIENT_FILE_UPLOAD_GET_DATA_CALLBACK getDataCallback, void* context)
{
    IOTHUB_CLIENT_RESULT result;
                                ...
                                else
                                {
                                    ...
                                    else
                                    {
                                        ...
                                        else
                                        {
                                            ...
                                            step2success = (Blob_UploadFromSasUri_WithCallback(STRING_c_str(sasUri), getDataCallback, context, &httpResponse, responseToIoTHub, handleData->certificates) == BLOB_OK);
                                            if (!step2success)
                                            {
