...
// Note: this applies only for the IoT Hub key-pair used for CSR, not for DPS authentication.
...
// Certificate Signing Request used when connecting to IoT Hub.
// A short-lived certificate based on this CSR will be issued by Device Provisioning Service.
static const char* x509csr = 
"-----BEGIN CERTIFICATE REQUEST-----\n"
"MIHNMHYCAQAwFDESMBAGA1UEAwwJZzFkZXZpY2UxMFkwEwYHKoZIzj0CAQYIKoZI\n"
"...""\n"
"0ZgGOiIg8WTVN/t/sQxRyw==\n"
"-----END CERTIFICATE REQUEST-----";

// Private Key used when connecting to IoT Hub.
...
static void register_device_callback(PROV_DEVICE_RESULT register_result, const char* iothub_uri, const char* device_id, void* user_context)
{
    ...
    else
    {
        ...
        if (register_result == PROV_DEVICE_RESULT_OK)
        {
            ...
            const char* certificate = Prov_Device_LL_Get_Issued_Client_Certificate(user_ctx->handle);
            if (certificate == NULL)
            {
...
int main()
{
    ...
    else if (Prov_Device_LL_Set_Certificate_Signing_Request(user_ctx.handle, x509csr))
    {
        (void)printf("failed calling Prov_Device_LL_Set_Certificate_Signing_Request\r\n");
    }
    else
    {
        ...
        // This option sets the registration ID and overrides the registration ID that is 
