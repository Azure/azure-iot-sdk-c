...
typedef struct PROV_INSTANCE_INFO_TAG
{
    ...
    tickcounter_ms_t last_send_time;
        ...
        else
        {
            ...
            else
            {
                ...
                else
                {
                    ...
                    (void)tickcounter_get_current_ms(result->tick_counter, &result->last_send_time);
                    result->last_send_time += (result->retry_after_value * 1000);
                }
            }
        }
    }
        ...
        if (prov_info->is_connected || prov_info->prov_state == CLIENT_STATE_ERROR)
        {
            ...
            switch (prov_info->prov_state)
            {
                case CLIENT_STATE_REGISTER_SEND:
                    ...
                    {
                        if ((current_time - prov_info->last_send_time) / 1000 > prov_info->retry_after_value)
                        {
                            ...
                            prov_info->last_send_time = current_time;
                        }
                    }
                    break;
