...
/** 
  @brief Function signarute for callback that is invoked by the Digital Twin SDK when a desired property is available from the service.
  ...
  @param[in] userInterfaceContext               Context pointer that was specified in the parameter *userInterfaceContext* during the device application's <c>DigitalTwin_InterfaceClient_Create</c> call.

  @return N/A.
*/
typedef void(*DIGITALTWIN_PROPERTY_UPDATE_CALLBACK)(const DIGITALTWIN_CLIENT_PROPERTY_UPDATE* dtClientPropertyUpdate, void* userInterfaceContext);
...
/** 
  ...
  @param[in] userInterfaceContext                   Context pointer that was specified in the parameter <c>userInterfaceContext</c> during the device application's <c>DigitalTwin_InterfaceClient_Create</c> call.

  @return N/A
*/
typedef void(*DIGITALTWIN_COMMAND_EXECUTE_CALLBACK)(const DIGITALTWIN_CLIENT_COMMAND_REQUEST* dtClientCommandContext, DIGITALTWIN_CLIENT_COMMAND_RESPONSE* dtClientCommandResponseContext, void* userInterfaceContext);
...
/** 
  ...
  @remarks Note that unlike many SDK functions, where the <c>userInterfaceContext</c> would be tied to a specific operation and then never referenced again, in 
           this case <c>userInterfaceContext</c> used throughout the life cycle of the interface handle.  In particular it is used when the interface's registration
           status changes.  It is also used when commands and properties are invoked on this interface, assuming the interface accepts such callbacks.  This allows
           the context to store state for the device application that spans the life of the interface and is not tied to a single call.
  ...
  @param[in]  userInterfaceContext              (Optional) Context pointer passed to dtInterfaceRegisteredCallback function on interface change, command updates, and property updates.
...
/** 
  ...
  @returns  A DIGITALTWIN_CLIENT_RESULT.
*/
MOCKABLE_FUNCTION(, DIGITALTWIN_CLIENT_RESULT, DigitalTwin_InterfaceClient_SetPropertiesUpdatedCallback, DIGITALTWIN_INTERFACE_CLIENT_HANDLE, dtInterfaceClientHandle, DIGITALTWIN_PROPERTY_UPDATE_CALLBACK, dtPropertyUpdatedCallback);
...
/** 
  ...
  @param[in] dtInterfaceClientHandle            Handle for the interface client.
  @param[dtPropertyUpdatedCallback]             Function pointer to invoke the callback on when commands for this interface arrive. 

  @returns  A DIGITALTWIN_CLIENT_RESULT.
*/
MOCKABLE_FUNCTION(, DIGITALTWIN_CLIENT_RESULT, DigitalTwin_InterfaceClient_SetCommandsCallback, DIGITALTWIN_INTERFACE_CLIENT_HANDLE, dtInterfaceClientHandle, DIGITALTWIN_COMMAND_EXECUTE_CALLBACK, dtCommandExecuteCallback);
...
/** 
  ...
  @param[in] userInterfaceContext (Optional) Context pointer passed to telemetryConfirmationCallback function when it is invoked.
...
/** 
  ...
  @param[in] userInterfaceContext (Optional) Context pointer passed to dtReportedPropertyCallback function when it is invoked.
...
MOCKABLE_FUNCTION(, DIGITALTWIN_CLIENT_RESULT, DigitalTwin_InterfaceClient_UpdateAsyncCommandStatusAsync, DIGITALTWIN_INTERFACE_CLIENT_HANDLE, dtInterfaceClientHandle, const DIGITALTWIN_CLIENT_ASYNC_COMMAND_UPDATE*, dtClientAsyncCommandUpdate);
