...
DIGITALTWIN_CLIENT_RESULT DigitalTwin_DeviceClient_CreateFromDeviceHandle(IOTHUB_DEVICE_CLIENT_HANDLE deviceHandle, const char* modelID, DIGITALTWIN_DEVICE_CLIENT_HANDLE* dtDeviceClientHandle)
{
    DIGITALTWIN_CLIENT_RESULT result;
    IOTHUB_CLIENT_RESULT iothubClientResult;
    bool urlEncodeOn = true;

    if ((deviceHandle == NULL) || (modelID == NULL) || (dtDeviceClientHandle == NULL) )
    {
        LogError("DeviceHandle=%p, modelID=%p, dtDeviceClientHandle=%p", deviceHandle, modelID, dtDeviceClientHandle);
    ...
    else if ((iothubClientResult = IoTHubDeviceClient_SetOption(deviceHandle, OPTION_DT_MODEL_ID, modelID)) != IOTHUB_CLIENT_OK)
    {
