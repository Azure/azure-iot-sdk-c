...
- **Having at least 4KB set for incoming SSL max content length buffer**: For some TLS libraries, this may be a configurable option and default may have been set as **4KB** for low memory footprint devices. During TLS handshake, IoT Hub service will send Server Hello which includes IoT Hub server side certificates. During **renewal** of IoT Hub server side certificates, check will be made to prevent this exceeding 4KB limit so that existing devices which are set for 4KB limit continue to work as before after renewal.
